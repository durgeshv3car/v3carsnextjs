...
const CustomSelect: React.FC<CustomSelectProps> = ({ options, placeholder, onSelect }) => {
  ...
  const [selectedItem, setSelectedItem] = useState<CarBrand | null>(null)
  ...
  const handleSelect = (item: CarBrand) => {
    setSelectedItem(item)
    setSearchTerm('')
    setIsOpen(false)
    if (onSelect) onSelect(item.brandId)
  }
    ...
    <div className="w-full relative" ref={dropdownRef}>
      ...
      >
        <p className="dark:text-white text-black">
          {selectedItem?.brandName || placeholder}
        </p>
      ...
      {isOpen && (
        <div
          className={`absolute top-full border border-gray-300 rounded-b-lg dark:border-[#2E2E2E] border-t-0 w-full z-30 max-h-[200px] overflow-y-auto ${styles['dropdown']}`}
        >
          <input
            type="text"
