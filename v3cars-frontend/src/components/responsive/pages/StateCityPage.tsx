...
export default function StateCityPage({ type, citySlug, fuelType }: PageProps) {
    ...
    function handleFuelType(tab: Tab) {
        if (tab.fuel === "Petrol") {
            router.push(`/${type}/${tab?.fuel?.toLowerCase()}-price-in-${citySlug}`);
        }
        if (tab.fuel === "Diesel") {
            router.push(`/${type}/${tab?.fuel?.toLowerCase()}-price-in-${citySlug}`);
        }
    ...
    return (
        <>
            ...
            <SearchSection type={capitalize(fuelType)} city={citySlug} state={type} setCityId={setCityId} cityId={cityId} selectState={selectState} setSelectState={setSelectState} />
                ...
                <div className="w-full lg:app-container py-6 mx-auto space-y-7">
                    <div className="flex flex-col lg:flex-row justify-between gap-5 w-full">
                        <div className="w-auto lg:min-w-[74%] space-y-6">
                            ...
                            <FuelPrices fuelData={metroCityFuelByCity} type={capitalize(fuelType)} />
