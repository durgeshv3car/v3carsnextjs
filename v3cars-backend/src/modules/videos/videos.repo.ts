...
export class VideosRepo {
  /** Today (per type): most recent up to DB NOW() */
  ...
  /** Latest (date desc); optionally exclude an id */
  ...
  /** Trending (no NOW() constraint in your PHP patterns): last_15days_view desc */
  async listTrending(videoType: number, limit = 9) {
    return prisma.tblwebvideos.findMany({
      where: { videoType, status: true, publishStatus: 1 },
