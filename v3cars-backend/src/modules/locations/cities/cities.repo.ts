...
import type { CitiesListQuery, MajorFuel } from './cities.types.js';

function buildWhere(q: CitiesListQuery): Prisma.tblcitiesWhereInput {
  const where: Prisma.tblcitiesWhereInput = {};

  if (q.q) where.cityName = { contains: q.q };
  if (typeof q.status === 'number') where.status = q.status;
  if (typeof q.stateId === 'number') where.stateId = q.stateId;
  if (typeof q.countryId === 'number') where.countryId = q.countryId;
  if (typeof q.isPopular === 'boolean') where.isPopularCity = q.isPopular ? 1 : 0;
  if (typeof q.isTop === 'boolean') where.isTopCity = q.isTop ? 1 : 0;

  if (q.majorFuel) {
    const map: Record<MajorFuel, keyof Prisma.tblcitiesWhereInput> = {
      petrol: 'ismajorCityPetrol',
      diesel: 'ismajorCityDiesel',
      cng: 'ismajorCityCNG',
    };
    (where as any)[map[q.majorFuel]] = 1;
  }

  return where;
}

function buildOrderBy(sortBy: CitiesListQuery['sortBy']): Prisma.tblcitiesOrderByWithRelationInput[] {
  switch (sortBy) {
