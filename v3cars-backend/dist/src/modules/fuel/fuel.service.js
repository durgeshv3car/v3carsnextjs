 ...
 */
async function resolveDistrictId(opts) {
    // 1) If a real tbldistricts id
    if (opts.districtId) {
        const found = await prisma.$queryRaw(Prisma.sql `
      SELECT id FROM tbldistricts WHERE id = ${opts.districtId} LIMIT 1
    `);
        if (found.length)
            return opts.districtId;
    }
    // 2) Map cityId -> districtId (state+name join)
    const tryMap = async (cityId) => {
...
export class FuelService {
    ...
    async metros(q) {
        ...
        const hasDays = Number.isFinite(q.days);
        ...
        return withCache(key, async () => {
            ...
            if (hasDays && days) {
                ...
                for (const r of raw) {
                    ...
                    if (r.fuelType === 1)
                        byCity[r.districtId].petrol.history.push(entry);
                    if (r.fuelType === 2)
                        byCity[r.districtId].diesel.history.push(entry);
                    if (r.fuelType === 3)
                        byCity[r.districtId].cng.history.push(entry);
                }
                return METRO_IDS
                    .map(id => byCity[id])
                    .filter(Boolean);
            ...
            for (const r of raw) {
                ...
                if (r.fuelType === 1)
                    byCity[r.districtId].petrol = block;
                if (r.fuelType === 2)
                    byCity[r.districtId].diesel = block;
                if (r.fuelType === 3)
                    byCity[r.districtId].cng = block;
            }
            return METRO_IDS
                .map(id => byCity[id])
                .filter(Boolean);
    ...
    async historyCombined(q) {
        ...
        return withCache(key, async () => {
            ...
            // per-day delta
            let prevP = null, prevD = null, prevC = null;
