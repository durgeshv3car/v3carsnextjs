...
export class ModelsController {
    ...
    async upcomingMonthlyCount(req, res) {
    ...
    async topSellingMonthly(req, res) {
    ...
    async priceList(req, res) {
        ...
        const data = await svc.priceList(id, q);
        res.json({ success: true, ...data });
    }
    async bestVariantToBuy(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelBestVariantQueryDto.parse(req.query);
        const data = await svc.bestVariantToBuy(id, q);
        res.json(data);
    }
    async dimensionsCapacity(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelDimensionsQueryDto.parse(req.query);
        const data = await svc.dimensionsCapacity(id, q);
        if (!data)
            return res.status(404).json({ success: false, message: 'Model not found' });
        res.json({ success: true, ...data });
    }
    async mileageSpecsFeatures(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelMsfQueryDto.parse(req.query);
        const data = await svc.mileageSpecsFeatures(id, q);
        res.json(data);
    }
    async prosCons(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
    ...
    async offersDiscounts(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelOffersQueryDto.parse(req.query);
        const data = await svc.offersDiscounts(id, q);
        res.json(data);
    }
    ...
    async othersOnSale(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelOthersQueryDto.parse(req.query);
        const data = await svc.othersOnSale(id, q);
        res.json(data);
    }
    // inside export class ModelsController
    async serviceCost(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
    ...
    async images(req, res) {
        ...
        const data = await svc.gallery(id, q);
        res.json(data);
    }
    async serviceCostpow(req, res) {
        const id = await this.resolve(req, res);
        if (!id)
            return;
        const q = modelServiceCostQueryDto.parse(req.query);
        const data = await svc.serviceCostpow(id, q);
        res.json(data);
    }
    async segmentTopSellingByMonth(req, res) {
