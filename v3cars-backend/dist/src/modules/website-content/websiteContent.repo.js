...
function buildInsuranceWhere(q) {
    const where = {};
    if (q.q) {
        where.OR = [
...
export class WebsiteContentRepo {
    ...
    async list(q) {
        ...
        if (q.moduleId === 3) {
            ...
            const mapped = rows.map((r) => ({ moduleId: 3, ...r }));
            return {
                rows: mapped,
                total,
                page: q.page ?? 1,
    ...
    /** Latest by module */
    async getLatestByModule(moduleId) {
        if (moduleId === 3) {
