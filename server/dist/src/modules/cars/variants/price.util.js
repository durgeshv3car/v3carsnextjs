 ...
 */
export function detectMultiplier(raw) {
    const s = raw.toLowerCase();
    if (s.includes('cr') || s.includes('crore'))
        return 10_000_000; // 1 Cr
    if (s.includes('lakh') || s.includes('lac') || s.includes('lk'))
        return 100_000; // 1 Lakh
    return 1; // assume absolute rupees when unit not present
}
export function extractNumbers(raw) {
...
export function extractPriceBand(raw) {
    if (!raw)
        return null;
 ...
 */
export function snapApproxINR(n) {
    ...
    if (frac >= 0.95)
        snappedLakhs = Math.ceil(inLakhs); // e.g. 4.96..4.99 → 5.00
