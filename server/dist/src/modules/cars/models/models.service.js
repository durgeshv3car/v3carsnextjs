...
function inBucket(min, max, bucket) {
    if (!bucket)
        return true;
...
const toYmd = (d) => (d ? new Date(d).toISOString().slice(0, 10) : undefined);
export class ModelsService {
    ...
    async list(q) {
        ...
        const cylinders = q.cylinders;
        const engineDisplacement = q.engineDisplacement;
        const mileage = q.mileage;
        const seating = q.seating;
        // multi-select lists (may come as arrays or comma-separated strings, validators should normalize them)
        const brandIdsList = q.brandIds;
        const bodyTypeIdsList = q.bodyTypeIds;
        const cylindersList = q.cylindersList;
        ...
        const key = cacheKey({
            ...
            launchMonth: q.launchMonth ?? undefined,
            launchFrom: toYmd(q.launchFrom),
            launchTo: toYmd(q.launchTo),
        });
        ...
        return withCache(key, async () => {
             ...
             * ------------------------------ */
            let allowedModelIdsArray;
            const hasPowertrainFilters = !!fuelType ||
                !!transmissionType ||
                !!cylinders ||
                !!cylindersList ||
                !!engineDisplacement ||
            ...
            if (q.priceBucket) {
                ...
                const filtered = withComputed.filter((m) => inBucket(m.computedMin ?? null, m.computedMax ?? null, q.priceBucket));
                // 5) Sort
                ...
                const brandIds = Array.from(new Set(pageRows.map((r) => r.brandId).filter((x) => typeof x === 'number')));
                ...
                const enriched = pageRows.map((m) => {
                    ...
                    const specs = specsMap.get(m.modelId) ??
                        {
                            powerPS: null,
                            torqueNM: null,
                            mileageKMPL: null,
            ...
            if (fuelType || transmissionType || cylinders || cylindersList || engineDisplacement || mileage) {
                ...
                const enriched = pageRows.map((m) => {
                    ...
                    const priceMin = (typeof m.expectedBasePrice === 'number' && m.expectedBasePrice > 0 ? m.expectedBasePrice : band.min) ?? null;
                    const priceMax = (typeof m.expectedTopPrice === 'number' && m.expectedTopPrice > 0 ? m.expectedTopPrice : band.max) ?? null;
                    return {
                        ...m,
                        brand: b ? { id: b.brandId, name: b.brandName, slug: b.brandSlug, logo: b.logoPath } : null,
                        priceMin,
            ...
            let rows = base.rows;
            const modelIds = rows.map((r) => r.modelId).filter((x) => typeof x === 'number');
            const brandIds = Array.from(new Set(rows.map((r) => r.brandId).filter((x) => typeof x === 'number')));
            const [brandRows, priceBands, specsMap, imageMap] = await Promise.all([
                brandsSvc.findByIds(brandIds),
        ...
        return withCache(key, async () => {
            ...
            if (m.segmentId) {
                const seg = await prisma.tblsegments
                    .findFirst({ where: { segmentId: m.segmentId }, select: { segmentName: true } })
                    .catch(() => null);
    ...
    async upcomingMonthlyCount(opts) {
        ...
        return withCache(key, async () => {
            ...
            const countsByKey = new Map(rows.map((r) => [r.bucket.slice(0, 7), Number(r.cnt) || 0]));
            const out = [];
    ...
    async topSellingModelsByMonth(opts) {
        ...
        return withCache(key, async () => {
            ...
            // 2) Brand names
            const brandIds = Array.from(new Set(modelRows.map(m => m.brandId).filter((x) => typeof x === 'number')));
            ...
            const bodyTypeIds = Array.from(new Set(modelRows.map(m => m.modelBodyTypeId).filter((x) => typeof x === 'number')));
            const bodyTypeRows = bodyTypeIds.length
                ...
                : [];
            const bodyTypeNameById = new Map(bodyTypeRows.map(bt => [bt.modelBodyTypeId, bt.modelBodyTypeName ?? null]));
            // 4) Segment name
            const segmentIds = Array.from(new Set(modelRows.map(m => m.segmentId).filter((x) => typeof x === 'number')));
            ...
            const rows = agg.map((r, idx) => {
                ...
                const bodyStyle = (m.modelBodyTypeId ? bodyTypeNameById.get(m.modelBodyTypeId) : null) ?? null;
                const segmentName = (m.segmentId ? segmentNameById.get(m.segmentId) : null) ?? null;
                const band = priceBands.get(m.modelId) ?? { min: null, max: null };
    ...
    async priceList(modelId, q) {
        ...
        return withCache(key, async () => {
            const ft = ftRaw;
            const tr = trRaw;
            const fuelNorm = ft === 'petrol' ? 'Petrol' :
                ft === 'diesel' ? 'Diesel' :
                    ft === 'cng' ? 'CNG' :
                        ft === 'hybrid' ? 'Hybrid' :
            ...
            let rows = (variants.rows || []).map(v => {
                ...
                const onRoad = q.cityId && typeof bandMin === 'number'
    ...
    async bestVariantToBuy(modelId, q) {
        ...
        return withCache(key, async () => {
            ...
            for (const [ptId, arr] of byPt.entries()) {
                ...
                if (wantFuel && (meta?.fuelType ?? '').toLowerCase() !== wantFuel)
                    continue;
                if (wantTrans && (meta?.transmissionType ?? '').toLowerCase() !== wantTrans)
                    continue;
                const rows = arr
                    .map(v => {
                    const priceMin = v.priceMin ?? null;
                    const vfmRaw = v.vfmValue;
                    ...
                    return {
                        ...
                        price: priceMin,
                        recommendation: v.variantRecommendation ?? '',
                        updatedDate: v.updatedDate ?? null,
                    };
                })
    ...
    async dimensionsCapacity(modelId, q) {
        const key = cacheKey({
        ...
        return withCache(key, async () => {
            ...
            if (!q?.detailed)
                return basePayload;
            // ---------- detailed sections ----------
            const toCm = (mm) => (typeof mm === 'number' ? mm / 10 : null);
            const toIn = (mm) => (typeof mm === 'number' ? +(mm / 25.4).toFixed(2) : null);
            const toFt = (mm) => (typeof mm === 'number' ? +(mm / 304.8).toFixed(2) : null);
            const conversions = {
            ...
            let tankByFuel = { Petrol: null, Diesel: null, CNG: null, Hybrid: null };
            ...
            if (pRows.length) {
                ...
                pRows.forEach(r => {
                    ...
                    if (!k)
                        return;
                    if (!map.has(k))
                        map.set(k, r.fuelTankCapacity ?? null);
                });
                tankByFuel = {
    ...
    async mileageSpecsFeatures(modelId, q) {
        ...
        return withCache(key, async () => {
            ...
            const powerStr = pt.powerPS ? `${pt.powerPS}PS${pt.powerMinRPM || pt.powerMaxRPM ? ` @ ${rpmRange(pt.powerMinRPM || null, pt.powerMaxRPM || null)}` : ''}` : null;
            const torqueStr = pt.torqueNM ? `${pt.torqueNM}Nm${pt.torqueMinRPM || pt.torqueMaxRPM ? ` @ ${rpmRange(pt.torqueMinRPM || null, pt.torqueMaxRPM || null)}` : ''}` : null;
            const sections = [
                {
    ...
    async fuelEfficiency(modelId, q) {
        ...
        return withCache(key, async () => {
            ...
            const ptIds = Array.from(new Set(variants.map(v => v.modelPowertrainId).filter((x) => typeof x === 'number')));
            ...
            const rows = variants
                ...
                if (wantFuel && (pt?.fuelType || '').toLowerCase() !== wantFuel)
                    return null;
                // transmission filter
                if (!transPass(pt?.transmissionType ?? null))
                    return null;
                const label = (pt?.powerTrain && pt.powerTrain.trim().length > 0)
                    ? pt.powerTrain
                ...
                return {
                    ...
                    claimedFE: pt?.claimedFE != null ? Number(pt.claimedFE) : null,
                    realWorldMileage: pt?.realWorldMileage != null ? Number(pt.realWorldMileage) : null,
    ...
    async offersDiscounts(modelId, q) {
    ...
    async monthlySales(modelId, q) {
        ...
        return withCache(key, async () => {
            ...
            const monthLabel = (y, m) => new Intl.DateTimeFormat('en-IN', { month: 'long', year: 'numeric' })
    ...
    async colours(modelId) {
        ...
        return withCache(key, async () => {
            ...
            try {
                ...
                const rows = await prisma.tblvariantcolors.findMany({
                    where: { modelId },
    ...
    async serviceCostpow(modelId, q // modelPowertrainId
    ) {
        const key = cacheKey({
        ...
        return withCache(key, async () => {
            ...
            const description = `This summary shows how scheduled service spend grows with time for the ${brand?.brandName ?? 'Brand'} ` +
    ...
    async segmentTopSellingByMonthFromPath(segRaw, q) {
        ...
        return withCache(key, async () => {
            ...
            if (!sales.length)
                return { segmentId, segmentName, year, month, rows: [], total: 0 };
            ...
            const brandIds = Array.from(new Set(models.map(m => m.brandId).filter(Boolean)));
