...
import type { RegisterRequest, LoginRequest, VerifyLoginRequest, UserResponse } from './auth.types.js';

const repo = new AuthRepo();

async function sendEmail(mailBody: string, subjectLine: string, receiverName: string, receiverEmail: string): Promise<void> {
  const customerData = {
...
export class AuthService {
  ...
  async verifyLogin(data: VerifyLoginRequest): Promise<UserResponse> {
    const user = await repo.findUserByEmail(data.emailAddress);
    if (!user) {
      throw new Error('User not found');
    }
