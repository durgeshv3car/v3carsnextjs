...
function buildPattern(tokens: string[]): string {
  if (!tokens.length) return '';
  return `%${tokens.join('%')}%`;
}

function buildPrefix(tokens: string[]): string {
  if (!tokens.length) return '';
  return `${tokens[0]}%`;
}

export class SearchRepo {
  async searchBrands(tokens: string[], limit: number): Promise<BrandRow[]> {
    if (!tokens.length) return [];
    const first = tokens[0];
    const pattern = `%${first}%`;
    const prefix = `${first}%`;

    return prisma.$queryRaw<BrandRow[]>(Prisma.sql`
  ...
  async searchModels(tokens: string[], limit: number): Promise<ModelRow[]> {
    if (!tokens.length) return [];
    const first = tokens[0];
    const pattern = `%${first}%`;
    const prefix = `${first}%`;
