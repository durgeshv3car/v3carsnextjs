...
export const rpmRange = (min?: number | null, max?: number | null) => {
  if (min && max && min !== max) return `${min} - ${max}rpm`;
  if (min || max) return `${min ?? max}rpm`;
  return null;
};

export function sum(ns: number[]) { return ns.reduce((a, b) => a + b, 0); }

export function buildAssetPath(name?: string | null): string | null {
  return name ?? null;
}
...
export function inBucket(min: number | null, max: number | null, bucket?: keyof typeof priceRanges): boolean {
...
export const toYmd = (d?: Date | string) => (d ? new Date(d).toISOString().slice(0, 10) : undefined);