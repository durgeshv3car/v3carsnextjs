...
export class VariantsService {
  async list(q: VariantsListQuery) {
    ...
    const key = cacheKey({
      ...
      powertrainId: (q as any).powertrainId ?? undefined,   // 🆕 if added in types/dto
    ...
    return withCache(key, async () => {
      ...
      if ((q as any).powertrainId) {
        powertrainIds = [(q as any).powertrainId];
      ...
      if (q.minPrice != null) {
        filtered = filtered.filter(
          (v) => (v.priceMax ?? v.priceMin ?? Infinity) >= q.minPrice!
        );
      }
      if (q.maxPrice != null) {
        filtered = filtered.filter(
          (v) => (v.priceMin ?? v.priceMax ?? 0) <= q.maxPrice!
        );
      }
