...
import VideoSideBarSkeleton from '../ui/skeleton/VideoSideBarSkeleton'
...
const CommonRecentVideo: React.FC<CommonRecentVideoProps> = ({ title, data }) => {
  return (
    <div className="w-full rounded-xl shadow-sm border border-gray-300 dark:border-[#262626] overflow-hidden">
      {/* Header */}
      ...
      <div className="flex flex-col divide-y divide-gray-200 dark:divide-[#262626]">
        {data.length > 0 ? data.slice(0, 6).map((video) => (
          <div
            key={video.id}
