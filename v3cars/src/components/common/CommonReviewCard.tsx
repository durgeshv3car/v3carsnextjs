...
import CarCardSkeleton from '../ui/skeleton/CarCardSkeleton'
        ...
        <section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 border p-2 rounded-lg dark:border-[#2E2E2E]">
                {data.length > 0 ? data.slice(0, 6).map((item) => {
                    // ✅ Remove inline color styles except on <a> tags
                    const sanitizedDescription = item.shortDescription.replace(
                        /<(?!a\b)([^>]+?)\sstyle="([^"]*?)color:[^;"]+;?([^"]*?)"/gi,
                        '<$1 style="$2$3"'
                    )
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex flex-col flex-grow space-y-2 p-3">
                                ...
                                <div
                                    className="line-clamp-3 text-sm/8 prose prose-invert max-w-none"
