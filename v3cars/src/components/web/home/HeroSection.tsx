...
import { clearBodyTypes, clearBrands, clearPriceBucket, setPriceBucket, toggleBodyType, toggleBrand } from '@/redux/slices/advanceSearchSlice';
...
const HeroSection: React.FC<HeroSectionProps> = ({ selectBrand, setSelectBrand, data, models }) => {
  ...
  useEffect(() => {
    if (activeTab === "budget") {
      setSelectSearchBrand(undefined);
      setModelId(null);
    } else {
      setBudget(undefined);
        ...
        {heroBanner.length > 0 ? (
          ...
          >
            {heroBanner.map((slide, index) => {
              const imageUrl =
                slide.imagePath && slide.imagePath.trim() !== ""
              ...
              return (
                <SwiperSlide key={index}>
                  {/* Click overlay */}
                  <div
                    className="absolute inset-0 z-10 bg-gradient-to-r from-black/35 to-transparent pointer-events-none"
                  />
          ...
          <div className="relative bg-slate-300 h-full w-full dark:bg-[#232323] animate-pulse" />
        )}
