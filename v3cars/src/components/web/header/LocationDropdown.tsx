...
const LocationDropdown: FC<LocationDropdownProps> = ({ location, setLocation, setHoverTab }) => {
  ...
  return (
    <div className="w-full lg:max-w-[1600px] bg-white dark:bg-[#171717] mx-auto shadow-md z-50 rounded-xl">
      {/* Top Banner */}
      <div className="bg-[#ffe380] h-[80px] w-full flex items-center justify-between relative rounded-t-xl">
        <Image
          ...
          alt="Cities"
          fill
          className="w-full h-full object-cover absolute top-0 left-0 opacity-50 rounded-t-xl"
        />
      </div>
              ...
              displayCities.map((city, index) => (
               ...
               <>
                  {[...Array(8)].map((_, i) => (
                    <div
                      key={i}
                      className="h-10 rounded border border-gray-200 bg-gray-300 dark:bg-[#292929] flex items-center justify-center animate-pulse dark:border-[#2e2e2e]"
                    />
                  ))}
                </>
            )}
