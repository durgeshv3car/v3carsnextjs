...
const PopularCar = () => {
  const { data: topSellingCarData, isLoading } = useGetTopSellingCarQuery({ limit: 25 });

  const topSellingCars: CarModel[] = topSellingCarData?.rows ?? [];

  return (
    <div className="flex flex-col gap-4">
      {!isLoading ? topSellingCars.slice(0, 2).map((car) => (
        <div
          key={car.rank}
      ...
      {!isLoading && topSellingCars.slice(2).map((car) => (
        <div
          key={car.rank}
          className="border border-gray-300 dark:border-[#2E2E2E] rounded-lg overflow-hidden bg-white dark:bg-transparent shadow-sm p-4 space-y-2"
        >
