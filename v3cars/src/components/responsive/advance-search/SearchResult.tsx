...
import { resetFilters, SelectOption } from '@/redux/slices/advanceSearchSlice'
...
import RankedCarSkeleton from '@/components/ui/skeleton/RankedCarSkeleton'
                        ...
                        {/* Applied Filter Chip */}
                        {
                            filters.map((filter, filterIdx) => {
                                ...
                                if (Array.isArray(filter.value)) {
                                    return filter.value.map((v, valueIdx) => (
                                        <div
                                            key={`${filterIdx}-${valueIdx}`}
                                            className="flex items-center gap-1 bg-white dark:bg-[#171717] text-sm font-medium px-4 py-2 rounded-md"
                                        >
                                            {v.label}
                                            <button
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <button
                                            className="hover:text-primary-hover"
                                            onClick={() => removeFilter(filter.key, filter.value as SelectOption)}
                                        >
                                            <BiX size={18} />
                                        </button>
                                    </div>
