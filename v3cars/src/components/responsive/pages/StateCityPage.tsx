...
import BasicTableSkeleton from "@/components/ui/skeleton/BasicTableSkeleton";
...
export default function StateCityPage({ type, citySlug, fuelType }: PageProps) {
            ...
            <div className="relative mb-[300px] sm:mb-[170px] lg:mb-14">
                <div className="absolute -top-12 left-0 w-full px-4 lg:px-10">
                    <div className="w-full lg:app-container mx-auto">
                        <PriceInputSection type={capitalize(fuelType)} city={citySlug} state={type} setCityId={setCityId} cityId={cityId} selectState={selectState} setSelectState={setSelectState} />
                    </div>
                </div>
            </div>
                ...
                <div className="w-full lg:app-container py-6 mx-auto space-y-7">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            {metroCityFuelByCity.length > 0 ? metroCityFuelByCity.map((item) => (
                                <FuelPrices
                                    key={item.cityName}
                                    type={fuelType}
                                    city={item.cityName}
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-5 w-full">
                        <div className="w-auto lg:min-w-[74%] space-y-6">
                            {
                                list10DaysPrice.length > 0 ? (
                                    ["Petrol", "Diesel", "CNG"].includes(capitalize(fuelType)) && (
                                        <Listof10DaysPrice type={fuelType} data={list10DaysPrice} city={citySlug} />
                                    )
                                ) : (
                                    <BasicTableSkeleton />
                                )
                            ...
                            <CityPriceTrend type={capitalize(fuelType)} city={citySlug} data={list10DaysPrice} />

                            <PriceTrends type={capitalize(fuelType)} city={citySlug} data={monthlyTrendsData ?? null} />

                            {
                                cityWiseFuelPrice.length > 0 ? (
                                    ["Petrol", "Diesel", "CNG"].includes(capitalize(fuelType)) && (
                                        <CityWiseFuelList type={fuelType} data={cityWiseFuelPrice} slug={citySlug} />
                                    )
                                ) : (
                                    <BasicTableSkeleton />
                                )
