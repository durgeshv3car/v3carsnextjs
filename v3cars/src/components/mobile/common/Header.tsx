  ...
  return (
    <>
      <header className="w-full bg-white dark:bg-[#171717] z-40 relative">
        ...
        {/* Search bar */}
        <div ref={searchRef} className="flex items-center mx-4 px-0.5 border dark:border-[#2E2E2E] rounded-full mb-2 bg-white dark:bg-[#171717] relative z-30">
          <input
            type="text"
          ...
          {searchOpen && debounced.length >= 2 && (
            <div
              className="absolute top-14 left-0 right-0 w-screen -mx-4 bg-white dark:bg-[#171717] border border-[#e5e5e5] dark:border-[#2e2e2e] rounded-2xl shadow-xl overflow-hidden"
        ...
        {/* Drawer Overlay */}
        <div
          className={`fixed inset-0 z-20 bg-black/80 transition-opacity duration-300 ${(isOpen || searchOpen || hoverTab) ? "opacity-100 visible" : "opacity-0 invisible"
            }`}
          onClick={() => {
            if (searchOpen) {
              setSearchOpen(false);
