...
import NewsListSkeleton from '@/components/ui/skeleton/NewsListSkeleton'
        ...
        <section >
            <div className="space-y-4">
                ...
                <div className="grid grid-flow-col auto-cols-[100%] sm:auto-cols-[48.90%] lg:auto-cols-[32.25%] gap-4 snap-x snap-mandatory overflow-x-auto scroll-smooth scrollbar-hide">
                    {chunkedNews.length > 0 ? chunkedNews.map((group, groupIndex) => (
                        <div
                            key={groupIndex}
                            className="snap-start border dark:border-[#262629] rounded-lg px-4 shadow flex flex-col justify-between divide-y dark:divide-[#2e2e2e]"
                        >
                            {group.map((item) => (
                                <div
                                    key={item.id}
